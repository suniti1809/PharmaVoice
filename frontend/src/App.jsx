...
const TABS = [
  ...
  },
  {
    ...
    subtitle:
      'AI risk scoring, severity assessment, root causes, CAPA & duplicate audit',
    icon: (
      <svg
        width="16"
        height="16"
  ...
  },
  {
    ...
    subtitle:
      'Historical registry, audit trail, and persistent complaint log',
    icon: (
      <svg
        width="16"
        height="16"
...
export default function App() {
  ...
  // Always start from the top when the website is opened/refreshed
  useEffect(() => {
    ...
    const timer3 = setTimeout(scrollToTop, 1000);
  ...
  const openCount = register.filter(
    (row) => row.status !== 'Closed'
  ).length;

  const criticalCount = register.filter(
    (row) => row.initial_severity === 'Critical'
  ).length;
  ...
  return (
    <div className="shell">
      <aside className="sidebar">
        <div className="brand">
          ...
          <span className="brand-sub">
            Complaint Intake &amp; Quality Assurance
          </span>
        </div>
              ...
              >
                ...
                {tab.id === 'assessment' &&
                  risk?.risk_score !== undefined && (
                    <span className="side-link-badge badge-risk">
                      {risk.risk_score}
                    </span>
                  )}
        ...
        <div className="side-stats" aria-label="System Metrics">
          <div
            ...
            onKeyDown={(e) =>
              e.key === 'Enter' && setActiveTab('register')
            }
          >
          ...
          <div
            ...
            onKeyDown={(e) =>
              e.key === 'Enter' && setActiveTab('assessment')
            }
          >
            <span className="stat-value">
              {risk?.risk_score ?? '—'}
        ...
        <div className="side-footer">
          ...
          <span className="crumb">
            Quality › Customer Complaints › {currentTab.shortLabel}
          </span>
        </div>
      </aside>
      ...
      <div className="workspace">
        <header className="workspace-header">
          <div className="workspace-title-group">
            <div className="workspace-crumb">
              Quality Assurance › {currentTab.title}
            </div>
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {tab.id === 'assessment' &&
                    risk?.risk_score !== undefined && (
                      <span className="tab-badge tab-badge-risk">
                        {risk.risk_score}
                      </span>
                    )}
        ...
        <div
          className="workspace-body tab-content"
          key={activeTab}
        >
          {activeTab === 'intake' && (
              ...
              {risk?.risk_score !== undefined && (
                <aside className="risk-banner-notice">
                  <div className="risk-notice-text">
                    ...
                    <div>
                      <strong>
                        AI Risk Evaluation Ready:
                      </strong>{' '}
                      Evaluated score is{' '}
                      <strong>
                        {risk.risk_score}/100
                      </strong>{' '}
                      ({risk.severity || risk.risk_band}).
                      CAPA recommendations and root causes
                      detected.
                    </div>
                  </div>
